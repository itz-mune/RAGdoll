...
export function ProfileSwitcher({ onNavigateToSettings }: ProfileSwitcherProps) {
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if (e.key === 'Escape') profileStore.getState().closeSwitcher();
    };
      ...
      <AnimatePresence>
        {switcherOpen && (
          <motion.div
            ...
            className="absolute bottom-full left-0 mb-2 z-50 w-64 rounded-xl border border-border/60 bg-popover shadow-xl"
          >
